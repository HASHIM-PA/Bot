...
// discord.js v14+ | Requires: ManageChannels permission
//
// Features:
//   • All channel types incl. Category, Thread, Private Thread, News Thread
//   • Category creation with same allow/deny permission controls
//   • Allow / Deny users & roles (bulk via mention or ID)
//   • Slowmode presets for text / forum / thread channels
//   • NSFW toggle for text / announcement channels
//   • Position (where in the channel list it appears)
//   • 3-step guided flow: Type → Settings → Permissions → Create
// ─────────────────────────────────────────────────────────────────────────────
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o.setName("topic")
        .setDescription("Topic / description (text & announcement channels)")
        .setRequired(false)
    )
    .addChannelOption((o) =>
      o.setName("category")
  ...
  // ── Component handler (buttons + selects) ─────────────────────────────────
  async handleComponent(interaction) {
    ...
    if (customId === "cc_type_select") {
      ...
      if (!hasExtras) {
        // Skip straight to permissions (or create if category with no perms needed)
    ...
    if (customId === "cc_extras_next") {
      ...
      if (t.isThread) {
        ...
        await _createChannel(interaction, session);
  ...
  async handleModal(interaction) {
    const { customId, user } = interaction;
    if (!customId.startsWith("cc_modal_")) return;

    const session = sessions.get(user.id);
    if (!session) return interaction.reply({ content: "⏱️ Session expired. Run `/createchannel` again.", ephemeral: true });
...
function buildExtrasPage(session) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0xfee75c)
    .setFooter({ text: "All settings optional except thread parent." });
  ...
  const btns = [];
  if (t.supportsNSFW)
    btns.push(new ButtonBuilder().setCustomId("cc_nsfw_toggle").setLabel(`NSFW: ${session.nsfw ? "ON ✅" : "OFF ❌"}`).setStyle(session.nsfw ? ButtonStyle.Danger : ButtonStyle.Secondary));
  if (t.supportsPosition)
    btns.push(new ButtonBuilder().setCustomId("cc_set_position").setLabel("📌 Set Position").setStyle(ButtonStyle.Secondary));
  if (t.isThread)
    btns.push(new ButtonBuilder().setCustomId("cc_set_thread_parent").setLabel("🔗 Set Parent Channel").setStyle(ButtonStyle.Primary));

  btns.push(new ButtonBuilder().setCustomId("cc_extras_next").setLabel(t.isThread ? "✅ Create Thread" : "Next: Permissions →").setStyle(ButtonStyle.Success));
  btns.push(new ButtonBuilder().setCustomId("cc_cancel").setLabel("🗑️ Cancel").setStyle(ButtonStyle.Danger));
...
function buildPermsPage(session) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      "Mention multiple users/roles at once in the popup. Press **✅ Create** when ready."
    )
    .setColor(0x57f287)
    .addFields(
...
async function _createChannel(interaction, session) {
  const guild = interaction.guild;
  const t     = getType(session.channelTypeValue);
  ...
  if (t.isThread) {
    ...
    if (!parent) {
      await interaction.editReply({ content: "❌ Parent channel not found.", embeds: [], components: [] });
      return;
    }
    try {
      ...
      await interaction.editReply({ embeds: [successEmbed(thread, session, t, interaction.user)], components: [] });
    } catch (err) { await errReply(interaction, err); }
    return;
  }
  ...
  try {
    ...
    await interaction.editReply({ embeds: [successEmbed(channel, session, t, interaction.user)], components: [] });
  } catch (err) { await errReply(interaction, err); }
}
...
function successEmbed(channel, session, t, user) {
...
async function errReply(interaction, err) {
  console.error("[createchannel]", err);
  await interaction.editReply({ content: `❌ Failed: \`${err.message}\``, embeds: [], components: [] });
}

function expiredReply(interaction) {
  return interaction.reply({ content: "⏱️ Session expired. Run `/createchannel` again.", ephemeral: true });
}
